package com.softserve.edu.dashboard.constants;

public class Messages {
	
	public static final String NOT_FILLED = "All fields must be filled!"; //Messages.NOT_FILLED
	public static final String INVALID_LOGN = "Invalid username or password!"; //Messages.INVALID_LOGN
	public static final String PASSWORDS_NOT_MATCH = "Passwords do not match!"; //Messages.PASSWORDS_NOT_MATCH
	public static final String LOGIN_UNAVAILABLE = "Login unavailable!"; //Messages.LOGIN_UNAVAILABLE



}
